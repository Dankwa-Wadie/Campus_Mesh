...
import com.campusmesh.android.sync.PacketIdUtil
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.campusmesh.android.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.campusmesh.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.campusmesh.android.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                com.campusmesh.android.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.campusmesh.android.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.campusmesh.android.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.campusmesh.android.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.campusmesh.android.model.NoisePayloadType.DELIVERED -> {
                ...
                com.campusmesh.android.model.NoisePayloadType.READ_RECEIPT -> {
                ...
                com.campusmesh.android.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                ...
                com.campusmesh.android.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.campusmesh.android.model.NoisePayload(
                type = com.campusmesh.android.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (clockSkewMs > ANNOUNCE_CLOCK_SKEW_TOLERANCE_MS) {
            ...
            return false
        } else if (clockSkewMs > com.campusmesh.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
        ...
        try {
            val neighborsOrNull = com.campusmesh.android.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = BitchatPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.campusmesh.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            if (rawContent.startsWith("[#") || rawContent.startsWith("[📢#")) {
                val closeBracket = rawContent.indexOf(']')
                if (closeBracket > 0) {
                    val ch = rawContent.substring(1, closeBracket)
                    channelName = if (ch.startsWith("📢#")) ch.substring(1) else ch
                    cleanContent = rawContent.substring(closeBracket + 1).trim()
                }
            }

            // Enforce role check for official announcements
            if (channelName == "#gctu-announcements" || cleanContent.startsWith("/alert ")) {
                val verifyPayload = if (cleanContent.startsWith("/alert ")) {
                    cleanContent.removePrefix("/alert ").trim()
                } else {
                    cleanContent
                }
                
                if (!com.campusmesh.android.crypto.RoleAttestationManager.verifyAnnouncement(verifyPayload)) {
                    Log.w(TAG, "🚫 Dropping unauthenticated announcement or alert from ${peerID.take(8)}")
                    return
                }
                
                // Promote all verified alerts to the official announcements channel
                channelName = "#gctu-announcements"
            }
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.campusmesh.android.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.campusmesh.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.campusmesh.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }
                ...
                val sys = com.campusmesh.android.model.BitchatMessage(
                    sender = "system",
