package com.campusmesh.android.ui.debug
...
import com.campusmesh.android.services.meshgraph.MeshGraphService
...
fun MeshTopologySection(
    ...
    val wifiAwareConnected by com.campusmesh.android.wifiaware.WifiAwareController.connectedPeers.collectAsState()
...
fun DebugSettingsSheet(
    ...
    val wifiAwareSupported by com.campusmesh.android.wifiaware.WifiAwareController.supported.collectAsState()
    val wifiAwareAvailable by com.campusmesh.android.wifiaware.WifiAwareController.available.collectAsState()
    val wifiAwareSupportStatus by com.campusmesh.android.wifiaware.WifiAwareController.supportStatus.collectAsState()
        ...
        if (isPresented) {
            ...
            while (true) {
                ...
                val devices = entries.map { (address, isClient, rssi) ->
                    ...
                    com.campusmesh.android.ui.debug.ConnectedDevice(
                ...
                try {
                    val ctrl = com.campusmesh.android.wifiaware.WifiAwareController
    ...
    BitchatBottomSheet(
        ...
        Box(modifier = Modifier.fillMaxWidth()) {
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        val running by com.campusmesh.android.wifiaware.WifiAwareController.running.collectAsState()
                        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                            ...
                            AssistChip(
                                onClick = { com.campusmesh.android.wifiaware.WifiAwareController.getService()?.sendBroadcastAnnounce() },
                                enabled = running,
                                label = { Text("Announce") }
                            )
                        }
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        val p = remember(gcsFpr) { com.campusmesh.android.sync.GCSFilter.deriveP(gcsFpr / 100.0) }
                        val nmax = remember(gcsFpr, gcsMaxBytes) { com.campusmesh.android.sync.GCSFilter.estimateMaxElementsForSize(gcsMaxBytes, p) }
