...
import com.campusmesh.android.ui.NotificationTextUtils
...
class MeshDelegateHandler(
                ...
                if (state.getJoinedChannelsValue().contains(message.channel)) {
                    ...
                    val viewingGeohash = try {
                        if (channel.startsWith("geo:")) {
                            ...
                            selected is com.campusmesh.android.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
                        } else false
    ...
    private suspend fun processPeerUpdate(mergedPeers: List<String>) {
        ...
        runCatching { com.campusmesh.android.services.MessageRouter.tryGetInstance()?.onPeersUpdated(mergedPeers) }
            ...
            if (isNostrAlias || isNoiseHex) {
                ...
                val canonical = com.campusmesh.android.services.ConversationAliasResolver.resolveCanonicalPeerID(
                    ...
                    nostrPubHexForAlias = { alias ->
                        ...
                        if (com.campusmesh.android.nostr.GeohashAliasRegistry.contains(alias)) {
                            com.campusmesh.android.nostr.GeohashAliasRegistry.get(alias)
                        } else {
                            // Best-effort: derive pub hex from favorites mapping for mesh nostr_ aliases
                            val prefix = alias.removePrefix("nostr_")
                            val favs = try { com.campusmesh.android.favorites.FavoritesPersistenceService.shared.getOurFavorites() } catch (_: Exception) { emptyList() }
                            favs.firstNotNullOfOrNull { rel ->
                                rel.peerNostrPublicKey?.let { s ->
                                    runCatching { com.campusmesh.android.nostr.Bech32.decode(s) }.getOrNull()?.let { dec ->
                ...
                if (canonical != currentPeer) {
                    ...
                    com.campusmesh.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, canonical, listOf(currentPeer))
                ...
                val favoriteRel = try {
                    ...
                    if (noiseKey != null) {
                        com.campusmesh.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                    } else null
                ...
                if (favoriteRel?.isMutual == true) {
                    ...
                    if (noiseHex != currentPeer) {
                        com.campusmesh.android.services.ConversationAliasResolver.unifyChatsIntoPeer(
                            state = state,
        ...
        mergedPeers.forEach { pid ->
            try {
                ...
                val npub = com.campusmesh.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKey)
                val tempNostrKey: String? = try {
                    if (npub != null) {
                        val (hrp, data) = com.campusmesh.android.nostr.Bech32.decode(npub)
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.campusmesh.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
     ...
     */
    fun getPeerInfo(peerID: String): com.campusmesh.android.mesh.PeerInfo? {
